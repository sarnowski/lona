...
fn spawn_expr(
    ...
    scheduler
        .spawn(realm, chunk_addr, ProcessId::NULL)
        .expect("spawn failed")
}
...
fn spawn_fails_when_pool_exhausted() {
    ...
    for _ in 0..max_spawn_attempts {
        if scheduler
            .spawn(&mut realm, chunk_addr, ProcessId::NULL)
            .is_none()
        {
            break;
        }
...
fn round_robin_three_processes() {
    ...
    let pid1 = scheduler
        .spawn(&mut realm, chunk1, ProcessId::NULL)
        .unwrap();
    let pid2 = scheduler
        .spawn(&mut realm, chunk2, ProcessId::NULL)
        .unwrap();
    let pid3 = scheduler
        .spawn(&mut realm, chunk3, ProcessId::NULL)
        .unwrap();
...
fn process_error_doesnt_affect_others() {
    ...
    let good_pid = scheduler
        .spawn(&mut realm, good_chunk, ProcessId::NULL)
        .unwrap();
    let bad_pid = scheduler
        .spawn(&mut realm, bad_chunk, ProcessId::NULL)
        .unwrap();
...
fn mixed_short_and_long_processes() {
    ...
    let short_pid = scheduler
        .spawn(&mut realm, short_chunk, ProcessId::NULL)
        .unwrap();
    ...
    let long_pid = scheduler
        .spawn(&mut realm, long_chunk, ProcessId::NULL)
        .unwrap();
