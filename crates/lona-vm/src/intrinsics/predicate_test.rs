...
fn is_nil() {
    ...
    call_intrinsic(id::IS_NIL, 1, &mut proc, &mut mem, &mut realm).unwrap();
...
fn is_int() {
    ...
    call_intrinsic(id::IS_INT, 1, &mut proc, &mut mem, &mut realm).unwrap();
    assert_eq!(proc.x_regs[0], Value::bool(true));
...
fn is_str() {
    ...
    proc.x_regs[1] = s;
    call_intrinsic(id::IS_STR, 1, &mut proc, &mut mem, &mut realm).unwrap();
    assert_eq!(proc.x_regs[0], Value::bool(true));
...
fn unknown_intrinsic() {
    ...
    let result = call_intrinsic(200, 0, &mut proc, &mut mem, &mut realm);
