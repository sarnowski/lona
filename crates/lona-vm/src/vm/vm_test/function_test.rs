...
fn eval_fn_creation() {
    ...
    let result = eval("(fn* [x] x)", &mut proc, &mut mem).unwrap();
    assert!(result.is_fn());
}
...
fn eval_fn_identity() {
    let (mut proc, mut mem) = setup();
    let result = eval("((fn* [x] x) 42)", &mut proc, &mut mem).unwrap();
...
fn eval_fn_with_body() {
    let (mut proc, mut mem) = setup();
    let result = eval("((fn* [x] (+ x 1)) 5)", &mut proc, &mut mem).unwrap();
...
fn eval_fn_two_args() {
    let (mut proc, mut mem) = setup();
    let result = eval("((fn* [a b] (+ a b)) 3 4)", &mut proc, &mut mem).unwrap();
...
fn eval_fn_three_args() {
    let (mut proc, mut mem) = setup();
    let result = eval("((fn* [x y z] (+ x (+ y z))) 1 2 3)", &mut proc, &mut mem).unwrap();
    assert_eq!(result, Value::int(6));
}
...
fn eval_fn_zero_args() {
    let (mut proc, mut mem) = setup();
    let result = eval("((fn* [] 99))", &mut proc, &mut mem).unwrap();
...
fn eval_fn_returns_nil() {
    let (mut proc, mut mem) = setup();
    let result = eval("((fn* [] nil))", &mut proc, &mut mem).unwrap();
...
fn eval_fn_predicate_true() {
    let (mut proc, mut mem) = setup();
    let result = eval("(fn? (fn* [] nil))", &mut proc, &mut mem).unwrap();
...
fn eval_fn_predicate_false_int() {
    let (mut proc, mut mem) = setup();
    let result = eval("(fn? 42)", &mut proc, &mut mem).unwrap();
...
fn eval_fn_predicate_false_keyword() {
    let (mut proc, mut mem) = setup();
    let result = eval("(fn? :keyword)", &mut proc, &mut mem).unwrap();
    assert_eq!(result, Value::bool(false));
}
...
fn eval_fn_arity_mismatch() {
    ...
    let result = eval("((fn* [a b] (+ a b)) 1)", &mut proc, &mut mem);
...
fn eval_fn_nested_call() {
    ...
    let result = eval("(+ ((fn* [] 10)) ((fn* [] 20)))", &mut proc, &mut mem).unwrap();
...
fn eval_fn_with_tuple_param() {
    ...
    let result = eval("((fn* [t] (count t)) [1 2 3])", &mut proc, &mut mem).unwrap();
...
fn eval_fn_with_map_param() {
    ...
    let result = eval("((fn* [m] (get m :a)) %{:a 42})", &mut proc, &mut mem).unwrap();
    assert_eq!(result, Value::int(42));
}
...
fn eval_closure_simple() {
    ...
    let result = eval("(((fn* [x] (fn* [y] (+ x y))) 10) 5)", &mut proc, &mut mem).unwrap();
...
fn eval_closure_is_fn() {
    ...
    let result = eval("(fn? ((fn* [x] (fn* [y] (+ x y))) 5))", &mut proc, &mut mem).unwrap();
    assert_eq!(result, Value::bool(true));
}
