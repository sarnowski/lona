...
fn eval_keyword_callable_not_found() {
    let (mut proc, mut realm, mut mem) = setup().unwrap();
    // (:missing %{:a 1}) → nil
    let result = eval("(:missing %{:a 1})", &mut proc, &mut realm, &mut mem).unwrap();
...
fn eval_keyword_callable_arity_error_zero_args() {
    ...
    let result = eval("(:a)", &mut proc, &mut realm, &mut mem);
    assert!(matches!(
        result,
...
fn eval_keyword_callable_arity_error_too_many_args() {
    ...
    let result = eval("(:a %{:a 1} :d :extra)", &mut proc, &mut realm, &mut mem);
    assert!(matches!(
        result,
        Err(RuntimeError::CallableArityError {
            expected: "1-2",
...
fn eval_keyword_callable_type_error_not_map() {
    ...
    let result = eval("(:a 42)", &mut proc, &mut realm, &mut mem);
    assert!(matches!(
        result,
...
fn eval_map_callable_not_found() {
    let (mut proc, mut realm, mut mem) = setup().unwrap();
    // (%{:a 1} :missing) → nil
    let result = eval("(%{:a 1} :missing)", &mut proc, &mut realm, &mut mem).unwrap();
    assert!(result.is_nil());
}
...
fn eval_map_callable_arity_error() {
    ...
    let result = eval("(%{:a 1})", &mut proc, &mut realm, &mut mem);
    assert!(matches!(
        result,
        Err(RuntimeError::CallableArityError {
            expected: "1-2",
...
fn eval_tuple_callable_out_of_bounds() {
    ...
    let result = eval("([10 20 30] 5)", &mut proc, &mut realm, &mut mem);
    assert!(matches!(
        result,
...
fn eval_tuple_callable_arity_error() {
    ...
    let result = eval("([1 2])", &mut proc, &mut realm, &mut mem);
    assert!(matches!(
        result,
        Err(RuntimeError::CallableArityError {
            expected: "1-2",
...
fn eval_tuple_callable_type_error_not_integer() {
    ...
    let result = eval("([1 2 3] :a)", &mut proc, &mut realm, &mut mem);
    assert!(matches!(
        result,
        Err(RuntimeError::CallableTypeError {
            callable: "tuple",
