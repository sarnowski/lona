...
impl<'src> Parser<'src> {
    ...
    #[inline]
    #[must_use]
    pub(super) const fn location(&self, span: Span) -> SourceLocation {
    ...
    #[inline]
    #[must_use]
    pub(super) const fn location_from(&self, start: usize, end: usize) -> SourceLocation {
    ...
    #[inline]
    pub(super) const fn spanned_with_trivia<T>(
        node: T,
        span: Span,
        trivia_start: usize,
    ) -> Spanned<T> {
    ...
    pub(super) fn advance(&mut self) -> Result<crate::token::Token<'src>, Error> {
    ...
    pub(super) fn expect_token(
        &mut self,
        expected: TokenKind,
    ) -> Result<crate::token::Token<'src>, Error> {
        let token = self.advance()?;
